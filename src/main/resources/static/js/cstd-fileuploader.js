		...
		CSTDuploader.prototype = {
			...
			initUploader:function(opts){	
				...
				uploader.on('stopUpload',function(a,b,c){	
					_self.btnDisabledRule('stoping');
				})
				// 所有文件上传完毕
				uploader.on('uploadFinished', function (file) {					
					_self.btnDisabledRule('finished');
				});	
				...
				uploader.on('uploadSuccess', function (file) {
					...
					$.post('http://localhost:8081/app/main/save',params,function(data){
				...
				uploader.on('uploadError', function (file) {					
					...
					var row = {
						...
						action: '<button id="repeatUploadBtn" type="button" class="btn btn-link attr-file-btn 	glyphicon glyphicon-repeat" data-toggle="tooltip" data-placement="top" title="重试"></button>'
					}			
					$("#uploadTable").bootstrapTable('updateByUniqueId', {id: id,row:row});				   
				});
			...
			initEvent: function(){
				...
				$("#uploadStartBtn").on('click',function(e){
					me.uploader.upload();
				})
				$("#uploadCleanBtn").on('click', function(e){
					// 移除队列中的所有文件
					me.uploader.reset();					
				})

				// 暂停上传			
				$("#uploadPauseBtn").on('click', function(e){
					// 暂停上传					
			...
			createDom : function(dom){							
				...
				var btnShow = dom.btnShow && true;
				if (btnShow) {
					component.append('<div id="attrToolbar"> <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#uploaderModal">上传附件</button></div>')
				}			
				component.append('<table id="attrTable"></table>')	
				component.append('<div id="uploaderModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">'+
						'<div class="modal-dialog upload-modal-dialog">'+
							'<div id="upload-content" class="modal-content">'+
								'<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>附件上传</div>'+
								'<div class="modal-body upload-modal-body">'+
									'<div id="uploadToolbar" class="text-right">'+
										'<div id="selectPicker">选择文件</div>'+									
										'<button id="uploadStartBtn" type="button" class="btn btn-primary btn-margin" disabled=true><span class="glyphicon glyphicon-open icon-margin"></span>上传</button>'+
										'<button id="uploadResumeBtn" type="button" class="btn btn-primary btn-margin" style="display:none"><span class="glyphicon glyphicon-play icon-margin"></span>开始</button>'+
										'<button id="uploadPauseBtn" type="button" class="btn btn-primary btn-margin" disabled=true><span class="glyphicon glyphicon-pause icon-margin"></span>暂停</button>'+																			
										'<button id="uploadCleanBtn" type="button" class="btn btn-danger btn-margin" disabled=true><span class="glyphicon glyphicon-trash icon-margin"></span>清空</button>'+									
									'</div>'+
			...
			btnDisabledRule:function(state){
				const btnRules = {
					// 队列中,等待上传的状态
				  	'waiting':{
				...
				/** 所有控制按钮 */
				var startBtn = $("#uploadStartBtn");
				var pauseBtn =	$("#uploadPauseBtn");
				var resumeBtn = $("#uploadResumeBtn");
				//var retryBtn = $("#uploadRetryBtn");
				var cleanBtn = $("#uploadCleanBtn");
				/** 按钮设置控制样式 */
			...
			loadAttrTable: function(config){
				...
				var tableOpts = {     
					// 数据获取路径
					method : 'get',
 			        url : _server +"/app/main/getFiles?dbName="+_params.dbName+"&className="+_params.className+"&oid="+_params.oid,   			        					         
                ...
                　　　　 { field: 'action', title: '操作', align:'center', width: 100,
                			events: window.operatEvents = {
				                'click .btn': function(e,value,row,index){
				                	...
				                	if(e.target.id==='downloadAttrBtn'){
				                		// 创建隐藏的可下载链接
				                	...
				                	} else if(e.target.id==='deleteAttrBtn'){
				                		$.post(_server + '/app/main/deleteFile',
				                		{ guid: guid },
				                		function(response){				                			
				                			var result = JSON.parse(response);
				                			if(result.success){
				                				$('#attrTable').bootstrapTable("removeByUniqueId", guid);
				                			}
				                		});
				                	}
                			...
                			formatter(value, row, index){
                				...
                				return	'<button id="downloadAttrBtn" type="button" class="btn btn-link attr-file-btn glyphicon glyphicon-download-alt" data-toggle="tooltip" data-placement="top" title="下载"></button>'+
                					'<button id="deleteAttrBtn" type="button" class="btn btn-link attr-file-btn glyphicon glyphicon-remove" style="color:#a94442 !important;" data-toggle="tooltip" data-placement="top" title="删除"></button>';
                			}
            			}
                　　 ]
                }
				$('#attrTable').bootstrapTable(tableOpts);
			},
			loadUploadTable: function(config){
                ...
                　　　　	{ field: 'action', align: 'right', title: '',  width: 80,
                			events: window.operatEvents = {
				                'click .btn': function(e,value,row,index){
				                	var guid = row.guid;
				                	// 上传失败重试当前文件
				                	if(e.target.id==='repeatUploadBtn'){
				                		debugger;				                		            						                	
				...
				$('#uploadTable').bootstrapTable(tableOpts);
			},
